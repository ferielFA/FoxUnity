        ...
        async function captureFace() {
            try {
                ...
                if (result.success) {
                    // Show success message
                    document.getElementById('faceStatus').innerHTML = 
                        '<i class="fas fa-check-circle"></i> Welcome back, ' + result.username + '! (Confidence: ' + result.confidence + ') Redirecting...';
                    document.getElementById('faceStatus').style.background = 'rgba(76, 175, 80, 0.9)';
                    document.getElementById('faceStatus').style.borderColor = '#4caf50';
                    
                    // Show success popup with confidence scores
                    let debugInfo = '';
                    if (result.debug && result.debug.all_scores) {
                        debugInfo = '<br><br><div style="text-align: left; font-size: 13px;">';
                        debugInfo += '<strong>📊 Match Scores:</strong><br>';
                        result.debug.all_scores.slice(0, 5).forEach(function(score, index) {
                            let color = score.is_match === 'YES' ? '#4caf50' : '#f44336';
                            debugInfo += (index + 1) + '. ' + score.username + ': ';
                            debugInfo += '<span style="color: ' + color + '; font-weight: bold;">Confidence ' + score.confidence + '</span>';
                            debugInfo += ' [' + score.is_match + ']<br>';
                        });
                        debugInfo += '</div>';
                    }
                    
                    showFaceResultPopup('success', 'Face Recognized!', 
                        'Welcome back, <strong>' + result.username + '</strong>!<br>Confidence Score: <strong>' + result.confidence + '</strong>' + debugInfo);
                    ...
                    // Show error message with confidence scores only
                    const errorMsg = result.message || 'No matching user found. Please try again.';
                    document.getElementById('faceStatus').innerHTML = 
                        '<i class="fas fa-times-circle"></i> Recognition failed';
                    document.getElementById('faceStatus').style.background = 'rgba(244, 67, 54, 0.9)';
                    ...
                    // Build message with confidence scores
                    let debugMsg = '<strong>' + errorMsg + '</strong>';
                    
                    // If we have debug data with scores, show them
                    if (result.debug && result.debug.all_scores && result.debug.all_scores.length > 0) {
                        debugMsg += '<br><br><div style="text-align: left; font-size: 13px;">';
                        debugMsg += '<strong>📊 Confidence Scores:</strong><br>';
                        debugMsg += '• Required: <span style="color: #ff9800; font-weight: bold;">' + result.debug.threshold + '+</span><br>';
                        debugMsg += '• Best Match: <span style="color: #00bcd4; font-weight: bold;">' + result.debug.best_match + ' (' + result.debug.best_confidence + ')</span><br><br>';
                        debugMsg += '<strong>🔍 All Users Tested:</strong><br>';
                        result.debug.all_scores.forEach(function(score, index) {
                            let color = score.is_match === 'YES' ? '#4caf50' : '#f44336';
                            debugMsg += (index + 1) + '. ' + score.username + ': ';
                            debugMsg += '<span style="color: ' + color + '; font-weight: bold;">Confidence ' + score.confidence + '</span>';
                            debugMsg += ' [' + score.is_match + ']<br>';
                        });
                        debugMsg += '</div>';
                    } else {
                        // No face detected at all
                        debugMsg += '<br><br><div style="text-align: center; color: #f44336;">';
                        debugMsg += '<strong>No face detected in captured image</strong><br>';
                        debugMsg += '<small>Please ensure your face is clearly visible and well-lit</small>';
                        debugMsg += '</div>';
                    }
