    ...
    public function save() {
        ...
        try {
            if ($this->id_satisfaction) {
                ...
                // Retourner l'ID pour cohérence avec l'insertion
                if ($result) {
                    return $this->id_satisfaction;
                } else {
                    $errorInfo = $query->errorInfo();
                    $errorMsg = isset($errorInfo[2]) ? $errorInfo[2] : 'Erreur inconnue';
                    error_log('❌ Erreur Satisfaction::save() UPDATE: ' . print_r($errorInfo, true));
                    throw new Exception('Erreur UPDATE: ' . $errorMsg);
                }
            } else {
                // Insertion
                $query = $db->prepare(
            ...
            if (strpos($errorMsg, "doesn't exist") !== false || 
                ...
                throw new Exception('TABLE_NOT_FOUND: La table satisfactions n\'existe pas. Exécutez create_satisfactions_table.php');
            }
            
            // Re-lancer l'exception pour qu'elle soit capturée plus haut
            throw $e;
        }
    }
